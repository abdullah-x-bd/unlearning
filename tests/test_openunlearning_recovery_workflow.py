...
def test_remote_handoff_contains_hash_verified_openunlearning_outputs():
    ...
    remote = remote.split('- name: Collect, verify, and freeze result bundle', 1)[0]
...
def test_results_are_frozen_and_uploaded_before_pod_termination():
    workflow = WORKFLOW.read_text()
    collect = workflow.split('- name: Collect, verify, and freeze result bundle', 1)[1]
    collect = collect.split('- name: Upload verified OpenUnlearning results before Pod termination', 1)[0]

    assert 'id: collect' in collect
    assert 'for attempt in 1 2 3' in collect
    assert '/workspace/recovery-handoff/.' in collect
    ...
    upload_pos = workflow.index('- name: Upload verified OpenUnlearning results before Pod termination')
    terminate_pos = workflow.index('- name: Terminate RunPod Pod')
    assert upload_pos < terminate_pos

    upload = workflow.split('- name: Upload verified OpenUnlearning results before Pod termination', 1)[1]
    upload = upload.split('- name: Terminate RunPod Pod', 1)[0]
    assert 'id: upload_results' in upload
    assert "steps.collect.outcome == 'success'" in upload
